...
void buble_down(struct heap *heap, int index){
    ...
    while((sol <heap->eleman_sayisi &&                  // heap özelliğini kontrol et
            heap->dizi[index].key < heap->dizi[sol].key) || 
           (sag <heap->eleman_sayisi && 
            heap->dizi[index].key < heap->dizi[sag].key)){
        if(sag>=heap->eleman_sayisi || heap->dizi[sol].key >heap->dizi[sag].key){ // sağ child yoksa veya sol child sağdan büyükse
